# Home Structure - YOU Soluciones Inmobiliarias

Referencia de contenido para la página de inicio (Next.js). Dominio de producción: https://yousoluciones.com/

## 1. Header

### Content
Logo: `/public/logo-you.svg`

### Navigation links
- Nosotros
- Propiedades
- Descargables
- Contacto

### Social links
- Facebook
- YouTube
- Instagram

### Notes
- Header incluye logo, navegación e iconos sociales.

## 2. Top Announcement

### Content
Text: Looking for accommodation or your next real state investment in México CLICK HERE

### Notes
- Text is in English.
- Contains typo: "real state" should likely be "real estate".

## 3. Hero Section

### Content
Headline: Find the perfect spot for YOU
Primary message:
SERVICIO PROFESIONAL INMOBILIARIO
CON SENTIDO HUMANO

### Assets
Imagen hero: definir en diseño / `/public`.

### Notes
- Main positioning combines English slogan with Spanish real estate service message.

## 4. Zones Section

### Content
Title: ENCUETRA EL INMUEBLE QUE ESTAS BUSCANDO EN LAS ZONAS DE MAYOR PLUSVALÍA LA CIUDAD.

### Zone items
- NUEVO POLANCO
- POLANCO
- SANTE FE
- CONDESA
- Cibeles+Roma+Norte
- BOSQUES

### Notes
- Typo detected: "ENCUETRA" should be "ENCUENTRA".
- Typo detected: "SANTE FE" should be "SANTA FE".
- Missing "DE" in "PLUSVALÍA LA CIUDAD"; likely should be "PLUSVALÍA DE LA CIUDAD".

## 5. Featured Properties Section

### Title
Propiedades Destacadas

### Properties

#### Property 1
Title: Renta Departamento de Lujo con Balcón en Polanco
Price: US$ 8,500.00
Address: Campos Elíseos, Polanco, Polanco II Secc, Miguel Hidalgo, 11550 Ciudad de México, CDMX, México
Status: En Renta
CTA: Tours Virtuales

#### Property 2
Title: Venta PH Dos Niveles con Terraza en Club de Golf Bosques de Santa Fe
Price: US$ 4,300,000.00
Address: Canadá 77, Parque San Andrés, Cuajimalpa de Morelos, 05600 CDMX, México
Status: En Venta
CTA: Tours Virtuales

#### Property 3
Title: Venta PH con Terraza en Miyana Torre Colibrí
Price: ,900,000.00
Address: Avenida Ejército Nacional, Granada, Ciudad de México, CDMX, México
Status: En Venta
CTA: Tours Virtuales

#### Property 4
Title: Renta PH con Terrazas y Balcones en Polanco
Price: ,000.00
Address: Eugenio Sue, Polanco IV Secc, Ciudad de México, CDMX, México
Status: En Renta
CTA: Tours Virtuales

### Notes
- Las destacadas son editables vía JSON de sitio y `/admin/listas`.

## 6. Virtual Tours Section

### Content
Title: Descubre Nuestras Experiencias 3D
CTA: Tours Virtuales

### Assets
Imagen / embed: configurar en env o CMS.

## 7. Property Owner CTA Section

### Content
Title: ¿Quieres vender o rentar tu propiedad?
CTA: Da Clic Aquí

## 8. Office Search Section

### Content
Title: ¿Estás buscando OFICINA?
Description:
Te ayudamos a encontrar el lugar perfecto para ti y tus colaboradores.

Supporting text:
Tenemos presencia en las mas solicitadas zonas corporativas de la Ciudad.

CTA: Busquemos tu nueva oficina

### Assets
Ilustración / foto: `/public` o diseño.

## 9. Footer

### Content
Tagline:
Comienza a escribir una nueva historia con nosotros.

Address:
Roberto Gayol 82-4 Int.2, Cd. Satélite, 53100 Naucalpan de Juárez, Méx.

Phone:
55-92-21-73-28

## 10. Copy Issues To Decide Later

- "real state" should likely be "real estate".
- "ENCUETRA" should be "ENCUENTRA".
- "SANTE FE" should be "SANTA FE".
- "plusvalía la ciudad" should likely be "plusvalía de la ciudad".
- "mas solicitadas" should be "más solicitadas".
